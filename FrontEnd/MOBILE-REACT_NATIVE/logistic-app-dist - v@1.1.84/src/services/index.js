...
export async function profileImage(ImagePicker, setImage){
     
    /**
     * url deste trecho de código
    ...
    /*
    url deste trecho de código
    https://www.youtube.com/watch?v=zoISjlKYUQ8
    **/
    ...
    uploadPhoto.on(
        "state_changed",
        snapshot => {
            // algoritmo para calculo de taxa percentual do download em tempo real
            const progress=(snapshot.bytesTransferred / snapshot.totalBytes) * 100
            return progress
            //aqui você salva o progress em uma variável de estado EX: setProgress(progress)
        },error => {
            alert(error)
        },
        ()=>{
            getDownloadURL(uploadPhoto.snapshot.ref).then(url =>{
                //aqui você pode setar a url do arquivo salvo no bucket do firebase
                //EX: setUrlImage(url)
                setImage(url)
            })
        }
...
/**
 * 
 *  regras do firebase automáticas
 * 
        rules_version = '2';
